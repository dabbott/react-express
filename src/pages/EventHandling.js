...
Notice how we define the \`handleClick\` function once, and reference it in the \`render\`. This is more ideomatic and performs better than our previous example with the function defined inline.
...
Tthere is nothing special about the name of the \`onClick\` prop passed to our \`Button\` - we could name it anything we want, so long \`Button\` passes that prop into a DOM component. For example, we could decide to name the event \`onPress\` and create our button \`Button\` as \`${`<Button onPress={() => ...} />`}\`. Within \`Button\`, we would then want to render \`${`<div onClick={this.props.onPress} />`}\`.
