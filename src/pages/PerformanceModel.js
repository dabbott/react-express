...
Component render | Fast | Components can re-render very frequently without a noticeable performance impact. It's common for \`render\` to be called many times as the \`state\` or \`props\` of a component change. This is part of the magic of React, but can also be confusing at first. In other JavaScript frameworks, you may have tried to reduce calls to \`render\` in order to limit DOM updates and improve performance. In React, reducing re-renders is something you only need to think about for particularly complex or expensive components. Note: when we say "render", this generally refers to the \`render\` method of your components getting called as part of the component lifecycle. This doesn't refer to actually updating the DOM, which React handles automatically based on what you return from your \`render\` methods.
\`shouldComponentUpdate\` | Very fast | In cases where component \`render\` is happening too frequently and causing performance issues, React gives you more control over the component lifecycle with \`shouldComponentUpdate\`, allowing you to choose exactly when a component does and doesn't re-render. When this method isn't defined, components will *always* re-render whenever their \`state\` or \`props\` change. Definie \`shouldComponentUpdate\` allows you to manually compare whatever data you need to determine whether or not the component should re-render. Return \`true\` to trigger a \`render\` call and \`false\` to prevent it.
