...
Some components support multiple children of a _specific_ type, rather than an [arbitrary](/patterns/arbitrary_children) type. For example, a tabs or accordian component take multiple children but only show one at a time. Let's look at a few of the ways we might impelment a `Tabs` component.
