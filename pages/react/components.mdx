...
Either way, after defining a component, e.g. `MyComponent`, we can then instantiate it as an **element** using JSX as `<MyComponent />`, which is equivalent to `React.createElement(MyComponent)`.
