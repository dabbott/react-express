...
`memo` | Very fast | In cases where a component is rendering too frequently and causing performance issues, we can use `memo` to reducing the frequency of component rendering. This function compares a component previous props to its new props before rendering the component — the component is only re-rendered if the props have changed.
