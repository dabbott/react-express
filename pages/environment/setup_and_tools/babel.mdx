...
Babel comes in two parts: the core, and plugins. Each individual language feature that Babel can transform, such as ES2015 classes, has a separate plugin. Collections of plugins are grouped into **presets**, so that you don't have to install and configure hundreds of individual dependencies.

## Which presets should I use?

You'll almost certainly want the `env` preset, which includes plugins for compiling newer JavaScript syntax into older JavaScript syntax. This plugin is highly configurable, letting you choose which browser versions you want to support.
