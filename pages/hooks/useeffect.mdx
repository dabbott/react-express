...
Here, we use `useEffect` to trigger an alert when the `count` is divisible by 3. The `callback` is called every time the `count` state changes because `count` is listed as a dependency.
