...
Here, we use `useEffect` to run an alert when the `count` is a multiple of 3. The `callback` is called every time the `isMultipleOf3` changes, since `isMultipleOf3` is listed as a dependency.
